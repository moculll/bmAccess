...
{	
	...
	public:
		...
		{
			if (this && *this && this->IsValid() && this->Data && this->Num())
			{
...
#pragma warning(suppress: 4244)
		...
		static wchar_t ReverseBytes(wchar_t wc) {
			return (wc >> 8) | (wc << 8);
		}
		inline std::string ToStringEx() const {
			if (this && *this && this->IsValid() && this->Data && this->Num()) {
				std::wstring WData(Data);

				// 使用 WideCharToMultiByte 进行转换
				int size_needed = WideCharToMultiByte(CP_UTF8, 0, WData.c_str(), (int)WData.size(), NULL, 0, NULL, NULL);
				std::string utf8Str(size_needed, 0);
				WideCharToMultiByte(CP_UTF8, 0, WData.c_str(), (int)WData.size(), &utf8Str[0], size_needed, NULL, NULL);
